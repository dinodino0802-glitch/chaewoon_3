...
// === 타이머 오디오 소스 설정 (CONFIG에서) ===
if (timerAudio) {
  timerAudio.src = CONFIG.timerSoundPath;
  console.log('✅ 타이머 음성 경로:', CONFIG.timerSoundPath);
}

// === 타이머 오디오 확인 ===
if (timerAudio) {
  console.log('✅ 타이머 오디오 로드됨:', timerAudio.src);
  timerAudio.volume = 1;
} else {
  console.error('❌ 타이머 오디오 요소를 찾을 수 없음');
}

// === 사용자 상호작용으로 오디오 권한 획득 ===
document.addEventListener('click', () => {
  if (timerAudio && timerAudio.paused) {
    timerAudio.play().catch(() => {
      // 오디오 재생 실패해도 무시 (권한 획득용)
    });
    timerAudio.pause();  // 즉시 일시정지
  }
}, { once: true });
